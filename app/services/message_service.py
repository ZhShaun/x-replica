...
def create_user_message(session: Session, user_name: str, user_message: str) -> UserMessage:
...
def get_paginated_messages(session: Session, offset: int, limit: int) -> tuple[List[Dict], bool, int | None]:
    # Use desc() function for ordering to resolve Mypy error
    query = select(UserMessage).order_by(desc(UserMessage.created_at)).offset(offset).limit(limit)
